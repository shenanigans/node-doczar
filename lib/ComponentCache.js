...
module.exports = function(){
    ...
    var getComponent = function (tpath) {
        ...
        if (!Object.hasOwnProperty (cleverRoot.property, name)) {
            cleverRoot.property[name] = pointer;
    ...
    Cache.resolve = function (tpath) {
        ...
        if (Object.hasOwnProperty.call (root.property, rootName)) // naturally rooted
            pointer = root;
        else if (Object.hasOwnProperty.call (cleverRoot.property, rootName))
            pointer = cleverRoot;
        else
    ...
    var dependencies = {};
    var inheritenceCalculated = false;
    var depCalls = {};
    Cache.setDependencies = function (pathstr, deps) {
        dependencies[pathstr] = deps;
    };

    Cache.getInherited = function (pathstr, callback) {
        var didSomething;
        do {
            didSomething = false;
            for (var component in dependencies)
                for (var i=0,j=dependencies[component].length; i<j; i++)
                    if (
                        !dependencies[dependencies[component][i]]
                     || !dependencies[dependencies[component][i]].length
                    ) {
                        didSomething = true;
                        dependencies[component].splice (i, 1);
                        i--; j--;
                    }
        } while (didSomething);
    };

...
function parseJSTypes (fname, fstr, defaultScope, baseNode, context, logger, processFile, sources) {
    ...
    function walkLevel (level, scope, thisNode, deadLine, scopeParent, globalScope) {
        ...
        function processCallExpression (expression, target) {
            ...
            if (
             ...
             && !Object.hasOwnProperty.call (scope, 'require')
                ...
                if (Object.hasOwnProperty.call (sources, modPathStr))
                    ...
                    sources[modPathStr] = new filth.SafeMap ({
            ...
            // is this an inline closure, like (function(){ /* ... */ })();
            // if (
            ...
            //     dummy['.arguments'] = expression.callee.params.map (function (param) {
            //         return new filth.SafeMap ({
            //             '.name':    param.name,
            //             '.types':   [],
            //             '.deref':   []
            //         });
            //     });
            //     for (var i=0,j=dummy['.arguments'].length; i<j; i++) {
            //         var arg = dummy['.arguments'][i];
            //         if (i < expression.arguments.length)
            //             divineTypes (arg, expression.arguments[i]);
            //         innerScope[arg['.name']] = arg;
            //     }
            //     dummy['.scope'] = innerScope;
            //     var localDeadLine = deadLine;
            //     for (var i=0,j=expression.callee.body.body.length; i<j; i++) {
            //         walkLevel (
            //             expression.callee.body.body[i],
            //             innerScope,
            //             expression.callee.type == 'FunctionExpression' ?
            //                 dummy
            //               : thisNode || dummy,
            ...
            //             globalScope
            //         );
            //         localDeadLine = expression.callee.body.body[i].loc.end.line;
            //     }
            ...
            switch (value.type) {
                ...
                case 'ArrowFunctionExpression':
                    ...
                    if (node['.arguments']) {
                        ...
                        node['.arguments'] = value.params.map (function (param) {
                            return new filth.SafeMap ({
                                '.name':    param.name,
                                '.types':   [],
                                '.deref':   []
        ...
        function getNode (initialPointer, level) {
            var pointer = initialPointer;
            if (level.computed)
                return;
            switch (level.type) {
                case 'MemberExpression':
                    pointer = getNode (pointer, level.object);
                    if (!pointer)
                        return;
                ...
                case 'Identifier':
                    ...
                    if (pointer['.deref'].length) {
                        if (pointer['.deref'].length > 1) {
        ...
        switch (level.type) {
            ...
            case 'FunctionDeclaration':
                // step into the name of the function
                if (level.id) {
            ...
            case 'IfStatement':
                ...
                break;
            case 'ForStatement':
                // register anything relevant in the init
            ...
            default:
                ...
                if (level.type == 'ConditionalExpression')
                    console.trace();
                logger.debug (
        ...
        switch (level.type) {
            case 'ImportDeclaration':
                ...
                if (Object.hasOwnProperty.call (sources, modPathStr))
                    ...
                    moduleNode = sources[modPathStr] = newNode (modPathStr);

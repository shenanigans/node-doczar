...
Component.prototype.finalize = function (warnings, options, callback) {
    ...
    async.each (children, function (child, callback) {
        ...
        process.nextTick (function(){
            ...
            // locals
            var locals = sourceDoc.local || self.local;
            for (var key in locals) {
                var child = locals[key];
                if (
                    (showNormal && !child.isDevelopment)
                 || (showDev && child.isDevelopment)
                 || (showAPI && child.isApi)
                    ...
                    if (!matchPaths (localChild.source, self.path))
                        localChild.isInherited = true;
                    var dest = child.final.isFunction ? 'localFunctions' : 'localValues';
                    self.final[dest].push (localChild);
                }
            }
            self.final.localValues.sort (componentSorter);
            self.final.localFunctions.sort (componentSorter);
            ...
            if (self.aliasTo) {
                console.log ('alias to '+self.aliasTo.path);
                self.final.aliasTo = self.aliasTo;
                for (var i in ALIAS_PROPS) {
                    var key = ALIAS_PROPS[i];
                    if (key == 'summaryDoc')
                        console.log ('summary', self.aliasTo.final[key]);

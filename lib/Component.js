...
Component.prototype.finalize = function (errors, warnings, options, callback) {
                    ...
                    if (
                     ...
                     || (showAPI && child.isApi)
                        ...
                        if (!matchPaths (localChild.source, self.path))
                            localChild.isInherited = true;
                        for (var j in self.interfaces)
                            if (Object.hasOwnProperty.call (self.interfaces[j].argument, key))
                                localChild.satisfies.push ({
                                    path:       self.interfaces[j].path,
                                    pathstr:    self.interfaces[j].pathstr,
                                });
                    ...
                    if (
                     ...
                     || (showAPI && child.isApi)
                        ...
                        if (!matchPaths (localChild.source, self.path))
                            localChild.isInherited = true;
                        for (var j in self.interfaces)
                            if (Object.hasOwnProperty.call (self.interfaces[j].returns, key))
                                localChild.satisfies.push ({
                                    path:       self.interfaces[j].path,
                                    pathstr:    self.interfaces[j].pathstr,
                                });
    ...
    for (var i in this.valtype) {
        ...
        try {
            var finalPath = this.context.resolve (thistype).path;
        ...
        if (this.valtype[i].generics.length) {
            ...
            for (var i in theseGenerics) {
                try {
                    var finalPath = this.context.resolve (theseGenerics[i]).path;
                    var finalStr = '';
                    delete finalPath[0][0];
                    for (var i in finalPath) finalStr += (finalPath[i][0] || '') + finalPath[i][1]
                } catch (err) {
                ...
                generics.push ({ name:finalStr, path:finalPath });
            }
        }

    ...
    fs.readFile (fname, function (err, buf) {
        ...
        while (fillMatches()) {
            ...
            if (match[2]) {
                ...
                for (var i=0,j=valtypeSelectorInfo.length-1; i<j; i+=3) {
                    var generics = !valtypeSelectorInfo[i+2] ? [] : valtypeSelectorInfo[i+2]
                        .split(',')
                        .map(function(z){
            ...
            do {
                ...
                if (inctype == 'signature') {
                    ...
                    for (var i=1,j=sigargSplit.length; i<j; i+=3) {
                        ...
                        sigvaltype = [];
                        for (var k=0,l=sigvaltypeSelectorInfo.length-1; k<l; k+=3) {
                            var generics = !sigvaltypeSelectorInfo[k+2] ? [] : sigvaltypeSelectorInfo[k+2]
                                .split(',')
                                .map(function(z){
                                    var frags = z.replace (/\s/g, '').split (Patterns.word);
                                    var outPath = [];
                        ...
                        insigargs.push ({ name:sigvalname, valtype:sigvaltype, path:sigargargpath });
                    }
                }
                    ...
                    for (var i=0,j=typeSelectorInfo.length-1; i<j; i+=3) {
                        var generics = !typeSelectorInfo[i+2] ? [] : typeSelectorInfo[i+2]
                            ...
                            .map(function(z){
                                ...
                                return { name:uglyPath.slice (1), path:outPath };
                            })
                            ;
                        ...
                        invaltype.push (
                            { path:valtypefrags, generics:generics, name:uglyValtype.slice (1) }
                        );
                    }
                }

    ...
    fs.readFile (fname, function (err, buf) {
        ...
        while (match = Patterns.tag.exec (fstr)) {
            ...
            if (match[2]) {
                ...
                for (var i=0,j=typeSelectorInfo.length-1; i<j; i+=3) {
                    var generics = !typeSelectorInfo[i+2] ? [] : typeSelectorInfo[i+2]
                        ...
                        ;
                    if (generics.length) console.log (generics);
                    // valtype.push ({ type:typeSelectorInfo[i+1], generics:generics });
                    valtype.push (typeSelectorInfo[i+1]);
                }
                valtype = valtype.join ('|');
            }
            var pathstr = match[3];
            var docstr = match[4];
            ...
            do {
                ...
                if (innerMatch[2]) {
                    invaltype = [];
                    var typeSelectorInfo = innerMatch[2].split(Patterns.typeSelectorWord);
                    for (var i=0,j=typeSelectorInfo.length-1; i<j; i+=3) {
                        var generics = !typeSelectorInfo[i+2] ? [] : typeSelectorInfo[i+2]
                            .split(',')
                            .map(function(z){
                        ...
                        invaltype.push (typeSelectorInfo[i+1]);
                    }
                    invaltype = invaltype.join ('|');
                }

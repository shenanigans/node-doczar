...
ComponentCache.prototype.getRelativeURLForType = function (start, type) {
    ...
    for (var i=sameFromRoot+1, j=type.length-1; i<j; i++) {
        var frag = type[i];
        ...
        str += childClass + '/';
        if (frag[1])
            str += encodeURIComponent (pointer.sanitaryName) + '/';
    }
    ...
    if (!pointer.hasChildren && pointer.ctype != 'spare')
        ...
        str += Patterns.ctypeDisambiguator[pointer.ctype] + '/';
        ...
        async.each (Object.keys (self.root), function (propname, callback) {
            ...
            if (
             ...
             || ( !options.showAPI && ( !prop.isDevelopment || options.showDev ) )
                ...
                prop.writeFiles (
                    path.join (basedir, prop.ctype == 'module' ? 'module' : 'property', propname),
                    '../../',
                    options,
                    callback
                );

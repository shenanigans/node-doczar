...
var pathDelimit     = '[\\.#~\\(\\)]';
var cpath       = '[\\w$\\.#~\\(\\)]+';
var ctypes      = 'property|member|spare|module|class|interface|enum|throws';
var ctypesInner = 'property|member|spare|module|class|interface|enum|throws|'
                 + 'callback|argument|named|returns'
                 ;
var modtypes    = 'development|api|super|implements|public|protected|'
                 + 'private|abstract|final|volatile|optional|dgroup'
                 ;
...
//      + util.format (
//         '[ \\t]*@(%s)(?:/(%s(?:\\[%s\\(?:,\\s*%s)*\\])?(?:\\|%s(?:\\[%s\\(?:,\\s*%s)*\\])?)*))?',
//                   ctypes, cpath,  cpath,      cpath,         cpath,  cpath,      cpath
//      )
//      + util.format (
//         '[ \\t]+(%s)[ \\t]*\\r?[\\n\\r]',
//                  cpath
...
module.exports.innerTag = new RegExp (
     ...
     +  util.format (
            '@(%s|(?:%s(?:\\|%s)*))%s?[ \\t]*(%s)?',
               ctypesInner, cpath, cpath, tSel,        cpath
        )
     + '[ \\t]*\\r?[\\n\\r]((?:.|[\\n\\r])*)'
);
// module.exports.innerTag = new RegExp (

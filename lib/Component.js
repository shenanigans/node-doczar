...
Component.prototype.finalize = function (filters, callback) {
    ...
    async.each (children, function (child, callback) {
        ...
        process.nextTick (function(){
            ...
            if (spareSource.summary)
                self.final.summaryDoc = spareSource.summary.final.doc;
            if (spareSource.details)
                self.final.details = spareSource.details.final.doc;
...
Component.prototype.inherit = function (loops) {
    ...
    if (this.parent && this.parent.superClasses.length && this.isTotallyEmpty) {
        var propName = this.path[this.path.length-1][1];
        for (var i=0, j=this.parent.superClasses.length; i<j; i++) try {

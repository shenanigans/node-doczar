...
Component.prototype.submit = function (info) {
    if (info.modifiers && info.modifiers.length) console.log (info.modifiers);
    for (var key in info)
...
Component.prototype.finalize = function (errors, warnings, options, callback) {
    // establish the final callback first
    var self = this;
    var final = function (err) {
        if (err)
            return callback (err);

        // KEYWORD process inheritence
    ...
    for (var i in this.valtype) {
        ...
        valtypes.push ({
            name:   thistype,
            path:   self.context.getRelativeURLForType (self.path, valtypefrags)
        });
    }
    ...
    if (this.ctype == 'spare') {
        ...
        if (this.spare.details) {
            ...
            if (this.spare.summary)
                this.final.summary = this.spare.summary.final.doc;
            else
                this.final.summary = this.final.details;
        } else if (this.spare.summary)
            this.final.details = this.final.summary = this.spare.summary.final.doc;
        ...
        for (var key in this.property)
            if (
             ...
             || ( !options.showAPI && ( options.showDev || !this.property[key].isDev ) )
                ...
                this.final[this.property[key].valtypeStr == 'function' ? 'functions' : 'statics'].push (
                    this.property[key].final
                );
            else
                console.log (this.pathstr, options.showAPI, this.isAPI, this.hasAPI);
        ...
        for (var key in this.member)
            if (
             ...
             || ( !options.showAPI && ( options.showDev || !this.member[key].isDev ) )
                ...
                this.final[this.member[key].valtypeStr == 'function' ? 'methods' : 'members'].push (
                    this.member[key].final
                );
...
/**     @Function Component#writeFiles

@argument/String basedir
    A directory path for this `Component` to write its files into. It will be immediately mkdirp'd.
@argument/String baseTagPath
    Injected into the index.html as `<base href={baseTagPath} />`. Consists of "../" repeated a
    number of times.
@callback
    @argument/Error
*/
